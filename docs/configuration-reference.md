...
| interceptors    | array of `MatomoRouterInterceptor` constructors | <i>none</i>                           | Interceptors types to register. <br><b>Only available in `.forRoot()` options argument; otherwise should be provided using [`MATOMO_ROUTER_INTERCEPTORS`](../README.md#customize-anything).</b> <br>For more complex scenarios, it is possible to configure any interceptor by providing token `MATOMO_ROUTER_INTERCEPTORS` as `multi` provider(s).                                     |
